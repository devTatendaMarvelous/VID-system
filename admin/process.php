...
error_reporting(0);
	        ...
	        if(isset($_POST['payment'])){
			    ...
			    'http://localhost/vid-system/admin/payment.php?payment='.$userLoggedIn,

			    'http://localhost/vid-system/add-bookings.php');

    
    $user_details_query = mysqli_query($con, "SELECT * FROM admin WHERE email ='$userLoggedIn'");
    while ($user = mysqli_fetch_assoc($user_details_query)) {
        $stud_id = $user['admin_id'];
